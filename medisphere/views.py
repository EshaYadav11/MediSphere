...
def addnewdoctor(request):
    ...
    if request.method == 'POST':
       ...
       # pincode=request.POST.get('pincode')
        ...
        #pincode=request.POST.get('pincode')
        hospital_name = request.POST.get('hospital')  # Assuming hospital name is sent
        my_hospital, created = hospital.objects.get_or_create(name=name, phone_number=phone_number)
        context = {'my_hospital': my_hospital}
        ...
        # If phone_number is mandatory, validate it
        if not phone_number:
            # handle error or return a message to user
            pass
...
def my_send_mail(request):
    ...
    if offline_patients:
        ...
        for time,email,msg in request.session.get('offline') :
            ...
            # time = p[0]
            # email = p[1] 
            subject = 'medisphere Confirmation'
    ...
    if decline_patients:
        for time,email,msg in request.session.get('decline') :
            subject = 'medisphere Declination' 
